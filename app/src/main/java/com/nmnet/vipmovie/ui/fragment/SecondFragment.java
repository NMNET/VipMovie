...
public class SecondFragment extends ParentBaseSwipeFragment {
	...
	private ArrayList<SimpleCallLog> holdCallLog(List<SimpleCallLog> simpleCallLogs) {
		List<SimpleCallLog> newCallLogs = new ArrayList<>();
		SimpleCallLog simpleCallLogTemp = new SimpleCallLog();
		newCallLogs.add(simpleCallLogs.get(0));
		for (SimpleCallLog simpleCallLog : simpleCallLogs) {
			boolean isNew = true;
			for (SimpleCallLog newCallLog : newCallLogs) {
				if (newCallLog.getPhone().equals(simpleCallLog.getPhone())) {
					isNew = false;
					simpleCallLogTemp = newCallLog;
					break;
				}
			}
			if (isNew) {
				newCallLogs.add(simpleCallLog);
			} else {
				simpleCallLogTemp.setTimes(simpleCallLogTemp.getTimes() + 1);
			}
		}

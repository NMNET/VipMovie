...
import com.nmnet.vipmovie.ui.activity.CallLogDetailActivity;
...
public class SecondFragment extends ParentBaseSwipeFragment {
    ...
    private ArrayList<SimpleCallLog> mSimpleCallLogs = new ArrayList<>();
            ...
            mHandler = new Handler() {
                ...
                public void handleMessage(Message msg) {
                    ...
                    if (msg.what == CALL_LOG) {
                        ...
                        mSimpleCallLogs.addAll(simpleCallLogs);
                        mTvSearchCallLog.setText("点击搜索" + simpleCallLogs.size() + "条通话记录");
                        holdCallLog(simpleCallLogs);
    ...
    private void holdCallLog(List<SimpleCallLog> simpleCallLogs) {
        for (SimpleCallLog simpleCallLog : simpleCallLogs) {
            simpleCallLog.setTimes(1);
        }
        for (int i = 0; i < simpleCallLogs.size(); i++)  //外循环是循环的次数
        {
            for (int j = simpleCallLogs.size() - 1; j > i; j--)  //内循环是 外循环一次比较的次数
            {
                if (simpleCallLogs.get(i).getPhone().equals(simpleCallLogs.get(j).getPhone())) {
                    simpleCallLogs.get(i).setTimes(simpleCallLogs.get(i).getTimes() + 1);
                    simpleCallLogs.remove(j);
                }
            }
        }
    }
    ...
    private void setRecyclerView(ArrayList<SimpleCallLog> simpleCallLogs) {
        ...
        mRvCallLog.setAdapter(new GeneRecycleAdapter(simpleCallLogs, getContext()) {
            ...
            protected void onBindItemHolder(View view) {
                ...
                tvCallLogName.setText(simpleCallLog.getName() + "(" + simpleCallLog.getTimes() + ")");
                ...
                if ("未接".equals(type)) {
                    ...
                    tvCallLogStatus.setTextColor(Color.parseColor("#aaaaaa"));
                }
                ...
                view.setOnClickListener(new View.OnClickListener() {
                    ...
                    public void onClick(View v) {
                        if (simpleCallLog.getTimes() == 1) {
                            ContentResolerUtil.callPhone(simpleCallLog.getPhone(), getActivity());
                        } else {
                            startShowActivity(simpleCallLog.getPhone());
                        }
                    }
                });
            }
    ...
    private void startShowActivity(String phone) {
        ArrayList<SimpleCallLog> simpleCallLogs = new ArrayList<>();
        for (SimpleCallLog simpleCallLog : mSimpleCallLogs) {
            if (phone.equals(simpleCallLog.getPhone())) {
                simpleCallLogs.add(simpleCallLog);
            }
        }

        Intent intent = new Intent(getActivity(), CallLogDetailActivity.class);
        intent.putParcelableArrayListExtra("calllogdetail", simpleCallLogs);
